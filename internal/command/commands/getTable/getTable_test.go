...
func TestParseOptions(t *testing.T) {
	...
	getTCmd = new(getTable.GetTable)
	err = getTCmd.ParseOptions([]string{"127.0.0.1:1234", "content"})
	assert.Nil(t, err)
...
func TestPrintUsage(t *testing.T) {
	// should be equal
	var getTCmd *getTable.GetTable
	assert.Equal(t, getTCmd.PrintUsage(), "Usage: getcontacts")
